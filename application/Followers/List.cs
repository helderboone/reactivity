    ...
    public class List
    {
        ...
        public class Handler : IRequestHandler<Query, List<Profile>>
        {
            ...
            public async Task<List<Profile>> Handle(Query request, CancellationToken cancellationToken)
            {
                ...
                var usersFollowing = new List<UserFollowing>();
                ...
                switch (request.Predicate)
                {
                    case "followers":
                        {
                            usersFollowing = await queryable.Where(x => x.Target.UserName == request.Username).ToListAsync();

                            foreach (var follower in usersFollowing)
                            {
                    ...
                    case "following":
                        {
                            usersFollowing = await queryable.Where(x => x.Observer.UserName == request.Username).ToListAsync();
